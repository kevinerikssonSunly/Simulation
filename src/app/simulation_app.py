...
def plot_energy_stack_st_altair(df):
    df_plot = df.copy()
    df_plot = df_plot[["produced_energy", "battery_discharged", "battery_charged", "baseload"]].fillna(0)
...
with st.sidebar:
    with st.expander("Output field explanations"):
        ...
        st.markdown("**Missing energy VWAP, EUR/MWh** – Average spot price at which demand shortfalls (missing energy) occurred.")
        st.markdown("**Excess energy VWAP, EUR/MWh** – Average spot price for energy overproduced and exported to the grid.")

        st.markdown("**Demand, MWh** – Total annual energy demand as determined by the demand level (in MWh).")
        ...
        st.markdown("**Missing energy, MWh** – Total MWh of energy that was needed to meet demand but could not be delivered.")
